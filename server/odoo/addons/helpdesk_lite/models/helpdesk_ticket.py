...
class HelpdeskTicket(models.Model):
    ...
    name = fields.Char(string='Ticket No', readonly=True, required=True, copy=False, default=lambda self: _('New'))
    name_char = fields.Char(string='Ticket Summary')
    ...
    priority = fields.Selection([('0', 'Low'), ('1', 'Normal'), ('2', 'High'), ('3', 'Urgent')], 'Priority', index=True,
    ...
    status_cmt = fields.Text(string="Status Comment")
    emp_charge = fields.Many2one(comodel_name="hr.employee", string="Engineer in charge")
    ...
    def message_get_suggested_recipients(self):
        ...
        except AccessError:  # no read access rights -> just ignore suggested recipients because this imply modifying
            # followers
            pass
        return recipients
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('New')) == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('helpdesk_lite.ticket') or _('New')
    ...
    #     for time in self:
    #         timesh = time.env['om.timesheet'].search(['ticket_no', '=', time.name])
    #         if(timesh):
    #             print(timesh)
    #             time.timesheet = timesh
    #         else:
    #             time.timesheet = ''
